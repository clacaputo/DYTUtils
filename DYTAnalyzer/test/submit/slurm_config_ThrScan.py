...
with open(yml,'r') as f:
    ...
    for sample in samples:
        ...
        config.inputSandboxContent = ['ntuplizer_cfg.py']
        ...
        #--------------------------------------------------------------------------------
        config.inputParamsNames = ['inputFile', 'outputFile','psim']

        config.stageoutDir = '/nfs/user/ccaputo/DyT/slurm_outputs/ThrScanOptimization/'+folder+'/'+sample#'/job_array_${SLURM_ARRAY_JOB_ID}'
...
# extracting from the input filename the desired index and adding it to the output filename.
        ...
        cmsRun ntuplizer_cfg.py inputFileList=${inputFile} outputFile=${outputFile} psim=${psim} useSelections=False
